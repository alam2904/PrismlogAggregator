            ...
            try:
                for file in alog_file.alog_files_list:
                    with open(file, "r") as read_file:
                        record = [data for data in read_file.readlines() if re.search(msisdn,data, re.DOTALL)]
                        if record:
                            for data in record:
                                splited_data = datetime.strptime(data.split(']')[0].split("[")[1].split(" ")[0], "%d/%b/%Y:%H:%M:%S")
                                
                                if splited_data >= start_date and splited_data <= end_date:
                                    self.alog_record_list.append(data)
                                    logging.info('access log for: %s found in: %s', msisdn, file)
                        else:
                            logging.info('No access record found')
