...
class ConfigManager:
    ...
    def get_operator_url_map(self, operator_id):
        ...
        try:
            ...
            if configMap:
                urls.extend([row["OPERATOR_URL"] for row in configMap])
        ...
        try:
            ...
            for handler_id in handler_id_list:
                try:
                    ...
                    if configMap:
                        self.handler_info.append(configMap)
                    else:
        ...
        for params in issue_handler_task_type_map:
            try:
                ...
                if handler_id:
                    ...
                    self.handler_map.append(configMap)  
            except Exception as ex:
                logging.info(ex)
    ...
    def get_file_info(self):
        ...
        if file_ids:
            file_ids_str = ",".join("'" + fid + "'" for fid in file_ids)
            # file_ids_str = tuple(file_ids) if len(file_ids) > 1 else "('" + file_ids[0] + "')"
            logging.info("FILE_IDS: %s", file_ids_str)
